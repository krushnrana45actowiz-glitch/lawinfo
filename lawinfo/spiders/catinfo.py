...
# class CatinfoSpider(scrapy.Spider):
#     name = "catinfo"
...
#     # cat=[   "https://www.lawinfo.com/personal-injury/",
#     #         "https://www.lawinfo.com/medical-malpractice/",
#     #         "https://www.lawinfo.com/workers-compensation/",
#     #         "https://www.lawinfo.com/employment-law-employee/",
#     #         "https://www.lawinfo.com/social-security-disability/"]
...
#         states = response.xpath('//h2[@id="by-state-label"]/following-sibling::ul[1]//a')

#         for state in states:
#             yield {
#                 "state": state.xpath("./text()").get(),
#                 "url": response.urljoin(
#                     state.xpath("./@href").get()
#                 )
#             }

# execute("scrapy crawl catinfo -O states.json".split())
...
import os
import hashlib
from lawinfo.items import LawinfoItem

class CatinfoSpider(scrapy.Spider):
    name = "catinfo"

    def start_requests(self):
        url = "https://www.lawinfo.com/social-security-disability/"
    ...
    def save_gzip(self, response, folder="responses"):
        os.makedirs(folder, exist_ok=True)

        # SHA256 from URL
        safe_name = hashlib.sha256(response.url.encode("utf-8")).hexdigest()

        file_path = f"{folder}/{safe_name}.html.gz"
    ...
    def parse(self, response):
        ...
        self.save_gzip(response)
        states = response.xpath(
            '//h2[@id="by-state-label"]/following-sibling::ul[1]//a'
        )

        for state in states:
            item=LawinfoItem()
            item["state_name"] = state.xpath("./text()").get()
            item["state_url"] = response.urljoin(state.xpath("./@href").get())

            yield item
            
            # yield {
            #     "state": state.xpath("./text()").get(),
            #     "url": response.urljoin(state.xpath("./@href").get())
            # }
